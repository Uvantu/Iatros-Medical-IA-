...
export class KnowledgeBridge {
  ...
  ingestManifest(manifestPath) {
    if (!manifestPath || !fs.existsSync(manifestPath)) {
      return { imported: 0, skipped: 0, path: manifestPath ?? null };
    }
    let imported = 0;
    let skipped = 0;
    for (const line of fs.readFileSync(manifestPath, 'utf8').split(/\r?\n/).filter(Boolean)) {
      try {
        ...
        this.store.upsertSource({
          ...r,
          source_id: r.source_id || stableId(r.absolute_path, r.relative_path),
          title: r.title || r.name || path.basename(r.absolute_path || r.relative_path || 'source'),
          role: r.document_role || r.role || 'unknown',
          uri: r.absolute_path || r.relative_path
        });
        imported += 1;
      } catch {
        skipped += 1;
  ...
  ingestCertifiedKnowledgeBase(kbDir) {
    if (!kbDir || !fs.existsSync(kbDir)) {
      return {
        ...
        claims: 0,
        review: 0,
        integrity_manifest_present: false
      };
    }
    ...
    const integrity = read('integrity-manifest.json');
    if (integrity) {
      out.integrity_manifest_present = true;
      this.store.appendEvidence({
        event_id: stableId('integrity_manifest_observed', kbDir, JSON.stringify(integrity)),
        event_type: 'integrity_manifest_observed',
        epistemic_status: 'OBSERVED',
        payload: {
          kb_dir: kbDir,
          keys: Object.keys(integrity),
          note: 'Manifest observed. Schema-specific cryptographic verification is delegated to the certified-KB producer until its contract is published.'
        }
      });
    }
    ...
    const ingestFacts = (name, status) => {
      for (const fact of recordsFrom(read(name), ['facts', 'records', 'items'])) {
        ...
        if (!claimId) continue;
        this.store.appendEvidence({
          event_id: stableId('knowledge_claim_imported', claimId, status, name),
          event_type: 'knowledge_claim_imported',
          concept_id: fact.concept_id || fact.topic_id || null,
          source_ref: sourceId,
          epistemic_status: status === 'active' ? 'SOURCE_BACKED' : 'REVIEW_REQUIRED',
          payload: { claim_id: claimId, status, origin_file: name }
        });
        if (status === 'active') out.claims += 1;
        else out.review += 1;
      }
    };
